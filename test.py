...
for seed in range(48, 49):
    ...
    # 保存模型
    torch.save(model.state_dict(), f'ckpt/test/model_2_params_{seed}.pt')
    torch.save(embed_layer, f'ckpt/test/model_2_embed_layer_{seed}.pt')
    
# 保存训练结果
loss_table = pd.DataFrame(loss_table)
loss_table.to_csv(f'data/test/result_2.csv', index=False, encoding='utf-8-sig')